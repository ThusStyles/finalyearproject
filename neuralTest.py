...
def get_data():
    ...
    for x in all_images[0:training_limit]:
        training_labels.append(x[0])
        training_images.append(x[1])

    max_limit = len(all_images) if (training_limit + 5000 > len(all_images)) else  (training_limit + 5000)

    for x in all_images[training_limit:max_limit]:
        testing_labels.append(x[0])
        testing_images.append(x[1])
...
def next_batch(batch_size):
    ...
    if index_in_epoch > training_limit:
        # Finished epoch
        epochs_completed += 1
        # Shuffle the data
        perm = np.arange(training_limit)
        ...
        start = 0
        index_in_epoch = batch_size
        assert batch_size <= training_limit
    end = index_in_epoch
...
def plot_example_errors(cls_pred, correct):
    ...
    cls_true = testing_cls[incorrect]
...
def print_test_accuracy(show_example_errors=False,
    ...
    while i < num_test:
        ...
        images = testing_images[i:j]

        # Get the associated labels.
        labels = testing_labels[i:j]
